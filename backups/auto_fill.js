...
// Map based on internal LLM knowledge
const floorMap = {
  // Pangyo
  ...
  // Trade Center / The Hyundai Seoul
...
async function updateRestaurant(id, updates) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request('https://fdr-zeta.vercel.app/api/admin/data', {
      method: 'PATCH',
      headers: {
...
async function run() {
  for (const rest of restaurants) {
    const info = floorMap[rest.name];
    if (info) {
      console.log(`Updating ${rest.name}...`);
  ...
  console.log('Finished updating all known restaurants.');
}
